# -*- coding: utf-8 -*-
"""
Created on Sat Nov 15 18:42:14 2025
...
def main():
    ...
    if PARALLEL_WORKERS > 1 and len(dir_trabajo_list) > 1:
        with ProcessPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
            futures = [executor.submit(run_dir_trabajo, d) for d in dir_trabajo_list]
            for future in as_completed(futures):
                try:
                    future.result()
                except Exception as exc:
                    print(f"Error en proceso paralelo: {exc}")
    else:
